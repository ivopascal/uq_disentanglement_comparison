...
from tensorflow.python.data.ops.test_mode import TEST_MODE
...
def ood_class_detection(dataset_name, config):
    ...
    for architecture_func, uq_name in architectures:
        ...
        if TEST_MODE:
            plt.savefig(f"../../figures/ood_class/histograms_{uq_name}_{dataset_name}_TEST.pdf")
        else:
            plt.savefig(f"../../figures/ood_class/histograms_{uq_name}_{dataset_name}.pdf")
